...
# db_hex <- db_hex |> 
#   mutate(fs_lable = case_when(
#     map_lgl(kursbeschreibung, ~ any(str_detect(.x, fixed(data_analytics_ki)))) ~ "Data Analytics",
#     map_lgl(titel, ~ any(str_detect(.x, fixed(softwareentwicklung)))) ~ "Softwareentwicklung",
#     map_lgl(kursbeschreibung, ~ any(str_detect(.x, fixed(nutzerzentriertes_design)))) ~ "Nutzerzentriertes Design",
#     map_lgl(kursbeschreibung, ~ any(str_detect(.x, fixed(it_architektur)))) ~ "IT-Architektur",
#     map_lgl(kursbeschreibung, ~ any(str_detect(.x, fixed(hardware_robotikentwicklung)))) ~ "Hardware/Robotikentwicklung",
#     map_lgl(kursbeschreibung, ~ any(str_detect(.x, fixed(quantencomputing)))) ~ "Quantencomputing",
#     TRUE ~ NA_character_
#   ))
